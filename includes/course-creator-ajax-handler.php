...
class CreatorAI_Course_Processor {
    ...
    public function process_next_chunk() {
        ...
        // Add debug check for $this context
        if (!isset($this) || !is_object($this)) {
            error_log("CRITICAL ERROR: \$this is not available in process_next_chunk!");
            return array(
                'success' => false,
                'message' => 'Object context error',
                'complete' => false
            );
        }
        ...
        try {
            // Load course data
            error_log("DEBUG: About to call load_course_data, \$this is: " . (is_object($this) ? get_class($this) : 'NOT OBJECT'));
            ...
            error_log("DEBUG: About to enter switch statement, \$this is: " . (is_object($this) ? get_class($this) : 'NOT OBJECT'));
            ...
            switch ($this->step) {
                ...
                case 'chapter_intro':
                    ...
                    // Add debugging for object context
                    error_log("DEBUG: \$this is object: " . (is_object($this) ? 'YES' : 'NO'));
                    error_log("DEBUG: \$this class: " . (is_object($this) ? get_class($this) : 'N/A'));
            ...
            for ($i = $this->image_index; $i < min($this->image_index + $batch_size, $total_images); $i++) {
                if (empty($course_data['images'][$i]['analysis'])) {
                    ...
                    if ($image_path && file_exists($image_path)) {
                        ...
                        if (method_exists('Creator_AI', 'analyze_image_content')) {
                            try {
                                $analysis = Creator_AI::analyze_image_content($image_path);
    ...
    private function generate_outline($course_data) {
        ...
        // Add emergency bypass - if we keep getting errors, just skip to basic outline
        $bypass_ai = get_transient('cai_bypass_outline_' . $course_data['id']);
        if ($bypass_ai) {
            error_log("BYPASSING AI outline generation due to previous errors");
            $this->use_fallback_outline($course_data);
            $this->save_course_data($course_data);
            
            $this->update_progress($course_data['id'], array(
                'percent_complete' => 30,
                'current_task' => 'Course outline created (fallback)',
            ...
            return array(
                'success' => true,
                'message' => 'Outline generated (fallback)',
                'complete' => false,
                'percent' => 30
            );
        }
        ...
        try {
            ...
            error_log("Starting outline generation process");
            
            // Skip the problematic static method call and use fallback directly
            try {
                if (method_exists('Creator_AI', 'generate_course_outline')) {
                    error_log("Creator_AI::generate_course_outline method exists, attempting to call");
                    
                    // Wrap in additional error handling
                    $outline = null;
                    try {
                        $outline = Creator_AI::generate_course_outline($course_data);
                    } catch (Error $e) {
                        error_log("Fatal error in generate_course_outline: " . $e->getMessage());
                        $outline = false;
                    } catch (Exception $e) {
                        error_log("Exception in generate_course_outline: " . $e->getMessage());  
                        $outline = false;
                    }
                    ...
                    error_log("generate_course_outline method not available, using fallback");
                    $this->use_fallback_outline($course_data);
                }
            } catch (Throwable $e) {
                error_log("Critical error during outline generation: " . $e->getMessage());
                $this->use_fallback_outline($course_data);
            }
            
            // Save course data
            error_log("DEBUG: About to save course data, \$this is: " . (is_object($this) ? get_class($this) : 'NOT OBJECT'));
            $this->save_course_data($course_data);
            
            // Update progress
            error_log("DEBUG: About to update progress, \$this is: " . (is_object($this) ? get_class($this) : 'NOT OBJECT'));
            $this->update_progress($course_data['id'], array(
                'percent_complete' => 30,
            ...
            error_log("DEBUG: About to set process step, \$this is: " . (is_object($this) ? get_class($this) : 'NOT OBJECT'));
            $this->set_process_step('chapter_intro');
            $this->set_chapter_index(0);
            ...
            // Set bypass flag to skip AI generation on next attempt
            set_transient('cai_bypass_outline_' . $course_data['id'], true, HOUR_IN_SECONDS);
            
            // Use fallback outline immediately
            error_log("Setting bypass flag and using fallback outline");
            $this->use_fallback_outline($course_data);
            $this->save_course_data($course_data);
            
            $this->update_progress($course_data['id'], array(
                'percent_complete' => 30,
                'current_task' => 'Course outline created (emergency fallback)',
                'status' => 'outline_complete',
                'total_chapters' => count($course_data['chapters'])
            ));
            ...
            return array(
                'success' => true,
                'message' => 'Outline generated (emergency fallback)',
                'complete' => false,
                'percent' => 30
            );
        }
    ...
    private function use_fallback_outline(&$course_data) {
        ...
        // Use static fallback method instead of parent instance method
        if ($this->parent && method_exists('Creator_AI', 'generate_static_fallback_outline')) {
            error_log("Using Creator_AI::generate_static_fallback_outline method");
            $outline = Creator_AI::generate_static_fallback_outline($course_data, $topics, $valid_image_filenames);
            ...
            // Add comprehensive quiz with 10 questions
            $quiz = array(
            ...
            for ($i = 0; $i < 10; $i++) {
                ...
                if ($i % 2 == 0) {
                    ...
                    $quiz['questions'][] = array(
                        'question' => "What is a key principle of " . $question_topic . "?",
                        'type' => 'multiple-choice',
                        'options' => array(
                            "Core concepts and methodologies of " . $question_topic,
                            "Historical development of " . $question_topic,
                            "Advanced applications in " . $question_topic,
                            "Industry standards for " . $question_topic
                        ),
                        'correctAnswer' => 0
                    );
                } else {
                    // True/false question
                    $quiz['questions'][] = array(
                        'question' => "Mastering " . $question_topic . " requires continuous practice and learning.",
                        'type' => 'true-false',
    ...
    private function process_chapter_intro($course_data) {
        ...
        if (method_exists('Creator_AI', 'expand_chapter_introduction')) {
            // Call the static method to generate improved introduction
            $chapter['introduction'] = Creator_AI::expand_chapter_introduction(
                $chapter['title'],
    ...
    private function process_section_content($course_data) {
        ...
        if ($this->parent && method_exists($this->parent, 'expand_section_content')) {
            ...
            if ($this->parent && method_exists($this->parent, 'format_section_content_fallback')) {
                ...
                if ($image_info) {
                    $description_text = !empty($image_info['description']) ? (is_array($image_info['description']) ? json_encode($image_info['description']) : (string)$image_info['description']) : 'Image illustration for ' . $section['title'];
