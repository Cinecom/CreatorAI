...
// ACTIONS ON PLUGIN ACTIVATION
    ...
    function creator_ai_activate() {
        ...
        if ( empty(get_option('cai_openai_tokens')) ) {
            update_option('cai_openai_tokens', 16384);
        }else{
            $current_tokens = get_option('cai_openai_tokens');
            update_option('cai_openai_tokens', intval($current_tokens));
        }
    }
...
// PLUGIN SETUP
    ...
    public function enqueue_assets($hook) {
       ...
       if (isset($_GET['page']) && $_GET['page'] === 'creator-ai-course-creator') {
        ...
        // Conditionally load debug assets
        if (get_option('cai_debug', false)) {
            wp_enqueue_style('cai-debug-css');
            wp_enqueue_script('cai-debug-js');
        }
...
// UTILITY
    ...
    public function sanitize_appearance_settings($settings) {
        ...
        if (isset($settings['sidebar_position'])) {
            $sanitized['sidebar_position'] = sanitize_text_field($settings['sidebar_position']);
        }
        
        // Add other appearance settings
        if (isset($settings['sidebar_behavior'])) {
            $sanitized['sidebar_behavior'] = sanitize_text_field($settings['sidebar_behavior']);
        }
        
        if (isset($settings['primary_color'])) {
            $sanitized['primary_color'] = sanitize_text_field($settings['primary_color']);
        }
