(function($) {
    ...
    const CONFIG = {
        ...
        progressPollInterval: 3000, // 3 seconds
        maxProgressPolls: 120 // 6 minutes max (120 * 3 seconds)
    };
    ...
    function openThumbnailReplacer() {
        ...
        if (!STATE.thumbnailReplacerFrame) {
            ...
            STATE.thumbnailReplacerFrame.on('select', function() {
                ...
                // Determine if this is a course directory image or external
                const courseImagePath = caiAjax.wp_upload_dir.baseurl + '/creator-ai-courses/';
                
                if (attachment.url.startsWith(courseImagePath)) {
                    // Image is in course directory - store just filename
                    const filename = attachment.url.replace(courseImagePath, '');
                    $('#cc-thumbnail-filename').val(filename);
                } else {
                    // Image is external - store full URL
    ...
    function getImageUrl(imageInput, size = 'full') {
        ...
        // Otherwise, construct the URL to the course directory
        const upload_dir = caiAjax.wp_upload_dir || { baseurl: site_url + '/wp-content/uploads' };
        return upload_dir.baseurl + '/creator-ai-courses/' + imageInput;
    }
